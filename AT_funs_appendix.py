...
#     df['bmfx'] = round(df['bm'].mul(df['ccy']),dgt).ffill()
#     if rebase == True:
...
def swings2(df,lvl,vlty_n,dgt,d_vol,dist_pct,r_vol,retrace_pct,bm_df,bm_col,rel = False):
    _o,_h,_l,_c = lower_upper_OHLC(df,relative= False)
    if rel == True:
        # df = relative(df=df,_o=_o,_h=_h,_l=_l,_c=_c, bm_df=bm_df, bm_col= bm_col, ccy_df=bm_df, 
        #                     ccy_col=ccy_col, dgt= dgt, start=start, end= end,rebase=True)
...
def regime(df,vlty_n,threshold,lvl,rel=False):   
